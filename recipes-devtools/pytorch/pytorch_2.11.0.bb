...
# Cap parallelism for the CUDA compile step. PyTorch's flash-attention
# kernels (third_party/flash-attention/csrc/flash_attn/src/flash_bwd_*.cu)
# instantiate large SM-8.0 templates and each nvcc subprocess can consume
# 4-8 GB of RAM at peak. Without this cap, the layer-wide
# PARALLEL_MAKE = "-j 10" in build/conf/local.conf would launch 10 such
